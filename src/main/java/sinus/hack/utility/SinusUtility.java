...
import static sinus.hack.core.manager.client.ConfigManager.IMAGES_FOLDER;
import static sinus.hack.features.modules.Module.mc;

public final class SinusUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = SinusHack.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/syncVersion121.txt").openStream())).readLine().equals(SinusHack.VERSION))
                SinusHack.isOutdated = true;
    ...
    public static void syncContributors() {
        try {
            ...
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                SinusHack.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            ...
            changeLog.add("Changelog [Recode; Date: " + SinusHack.BUILD_DATE + "; GitHash:" + SinusHack.GITHUB_HASH + "]");
            changeLog.add("\n");
