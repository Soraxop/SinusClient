package sinus.hack.core.manager.player;
...
import sinus.hack.events.impl.EventPostTick;
import sinus.hack.events.impl.PacketEvent;
import sinus.hack.events.impl.TotemPopEvent;
import sinus.hack.features.modules.Module;
import sinus.hack.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                SinusHack.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
