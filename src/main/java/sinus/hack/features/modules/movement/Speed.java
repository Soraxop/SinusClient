...
import sinus.hack.injection.accesors.IInteractionManager;
import sinus.hack.features.modules.Module;
import sinus.hack.setting.Setting;
import sinus.hack.utility.interfaces.IEntity;
...
import static sinus.hack.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                SinusHack.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && SinusHack.core.getSetBackTime() > 1000) {
            for (PlayerEntity ent : Managers.ASYNC.getAsyncPlayers()) {
                if (ent != mc.player && mc.player.squaredDistanceTo(ent) <= 2.25) {
                    float p = mc.world.getBlockState(((IEntity) mc.player).sinusHack$getVelocityBP()).getBlock().getSlipperiness();
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && SinusHack.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onTick(EventTick e) {
        ...
        if ((mode.is(Mode.GrimIce) || mode.is(Mode.GrimCombo)) && mc.player.isOnGround()) {
            BlockPos pos = ((IEntity) mc.player).sinusHack$getVelocityBP();
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            SinusHack.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
