...
public class ServerHelper extends Module {
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            if (spek.getValue()) {
                ...
                if (content.contains("спек") || content.contains("ызус") || content.contains("spec") || content.contains("spek") || content.contains("ызул")) {
                    String name = SinusUtility.solveName(pac.content().getString());
            ...
            if (photomath.getValue())
                if (pac.content().getString().contains("Решите: ") && Objects.equals(SinusUtility.solveName(pac.content().getString()), "FATAL ERROR"))
                    try {
            ...
            if (airDropWay.getValue() && pac.content().getString().contains("Аирдроп")) {
                try {
                    ...
                    SinusHack.gps_position = new BlockPos(xCord, yCord, zCord);
    ...
    public boolean check(String checkstring) {
        return checktimer.passedMs(3000) && (Objects.equals(SinusUtility.solveName(checkstring), "FATAL ERROR"));
    }
