package sinus.hack.features.modules;
...
import sinus.hack.core.manager.client.CommandManager;
...
import sinus.hack.features.modules.client.Windows;
import sinus.hack.features.modules.client.UnHook;
...
import static sinus.hack.SinusHack.LOGGER;
import static sinus.hack.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        SinusHack.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) SinusHack.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[SinusHack] enabled " + this.getName());
    ...
    public void disable() {
        try {
            SinusHack.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[SinusHack] disabled {}", getName());
