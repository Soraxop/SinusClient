...
import sinus.hack.features.modules.combat.AutoAnchor;
...
import sinus.hack.utility.render.animation.EaseOutCirc;
...
public class TargetHud extends HudElement {
    ...
    private final Setting<ModeEn> Mode = new Setting<>("Mode", ModeEn.SinusHack);
    ...
    public void onEnable() {
        try {
            custom = SinusUtility.getCustomImg("thud");
        } catch (Exception e) {
            sendMessage(".minecraft -> SinusHackRecode -> misc -> images -> thud.png");
        }
    }
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (animation.getAnimationd() > 0) {
            ...
            switch (Mode.getValue()) {
                case SinusHack -> renderSinusHack(context, health, animationFactor);
    ...
    private void renderSinusHack(DrawContext context, float health, float animationFactor) {
        float hurtPercent = target.hurtTime / 6f;
    ...
    public enum ModeEn {
        SinusHack, NurikZapen, CelkaPasta
    }
}